...
async def public_chat_stream(request: Request):
    ...
    try:
        ...
        # Check if this is a new session (no session UUID cookie)
        session_uuid = request.cookies.get("chatbot_session_id")
        is_first_message = not session_uuid

        if is_first_message:
            # Generate session UUID for first message
            timestamp = int(time.time() * 1000)
            random_suffix = ''.join(random.choices(string.ascii_lowercase + string.digits, k=10))
            session_uuid = f"session_{timestamp}_{random_suffix}"
            logger.info(f"🆕 Generating new session UUID for first message: {session_uuid}")
        ...
        # Pass session UUID to chatbot service via internal header (for database creation)
        headers["X-Internal-Session-UUID"] = session_uuid
        ...
        async with httpx.AsyncClient(timeout=30.0, follow_redirects=False) as client:
            ...
            # Session UUID is generated by API Gateway, not extracted from response
            if is_first_message:
                logger.info(f"🍪 Setting httpOnly cookie for new session: {session_uuid}")
