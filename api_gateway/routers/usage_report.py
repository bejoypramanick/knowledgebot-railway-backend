...
<div class="section"><h2>API Token Usage Log <span style="font-size:13px;color:var(--muted);font-weight:400">(actual Gemini usage per request)</span></h2><div class="table-wrap"><table>
  <thead><tr><th>Date</th><th>Session</th><th>Message</th><th>Provider</th><th>Model</th><th>Call Type</th><th>Prompt Tok</th><th>Completion Tok</th><th>Cache Read</th><th>Cache Write</th><th>Tools</th><th>Source</th><th>Est. Cost</th></tr></thead>
...
function render() {
  ...
  document.getElementById('token-log-table').innerHTML = tokenLog.slice(0,300).map(r => {
    ...
    return `<tr>
      <td>${fmtDateTime(r.created_at)}</td>
      <td class="mono">${trunc(r.session_id,18)}</td>
      <td class="mono">${trunc(r.message_id,18)}</td>
      <td>${r.provider||'-'}</td>
      <td>${r.model||'-'}</td>
      <td>${r.api_call_type||'-'}</td>
      <td class="token-cell">${fmt(r.prompt_tokens)}</td>
      <td class="token-cell">${fmt(r.completion_tokens)}</td>
      <td class="token-cell">${fmt(cache.read)}</td>
      <td class="token-cell">${fmt(cache.write)}</td>
