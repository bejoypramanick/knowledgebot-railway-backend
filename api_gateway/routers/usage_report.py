...
def _build_excel_style_usage_report(data, tenant_id=""):
    ...
    derivations = {
        10: "Formula",
        12: "Total = Mem + CPU + Vol + Egress + Embed + Obj. Mem = 2 GB x $0.00000386 x 18,000s. CPU = 2 vCPU x $0.00000772 x 18,000s. Egress = 0.15 GB x $0.05. Obj = 0.05 GB x $0.015.",
        13: "Total = Mem + CPU + Vol + Egress + Embed + Obj. Mem = 3 GB x $0.00000386 x 18,000s. CPU = 3 vCPU x $0.00000772 x 18,000s. Obj = 0.05 GB x $0.015.",
        14: "Total = Mem + CPU + Vol + Egress + Obj. Mem = 1 GB x $0.00000386 x 18,000s. CPU = 2 vCPU x $0.00000772 x 18,000s. Egress = 0.6 GB x $0.05.",
        15: "Total = Mem + CPU + Vol + Egress + Obj. Mem = 1 GB x $0.00000386 x 18,000s. CPU = 2 vCPU x $0.00000772 x 18,000s. Egress = 0.4 GB x $0.05.",
        16: "Knowledgebase total = row 12 + row 13 + row 14 + row 15.",
        17: "Chatbot seconds = total conversation seconds below.",
        18: "Total = Mem + CPU + Vol + Egress + LLM + Obj. Mem = 3 GB x $0.00000386 x conversation seconds. CPU = 2 vCPU x $0.00000772 x conversation seconds. Egress = 0.4 GB x $0.05. LLM = Grand total below.",
        20: "API Gateway seconds = conversation seconds + 18,000s.",
        21: "Total = Mem + CPU + Vol + Egress + Obj. Mem = 1 GB x $0.00000386 x API seconds. CPU = 1 vCPU x $0.00000772 x API seconds. Egress = 1 GB x $0.05.",
        23: "Postgres seconds = conversation seconds + 18,000s.",
        24: "Total = Mem + CPU + Vol + Egress + Obj. Mem = 1 GB x $0.00000386 x Postgres seconds. CPU = 1 vCPU x $0.00000772 x Postgres seconds. Vol = 1 GB x $0.00000006 x 2,592,000s.",
        27: "Total = Mem + CPU + Vol + Egress + Obj. Mem = 1 GB x $0.00000386 x 18,000s. CPU = 1 vCPU x $0.00000772 x 18,000s.",
        30: "Health monitor off: Mem = 0 GB x $0.00000386 x 435,000s. Vol = 0 GB x $0.00000006 x 2,592,000s. Total ~= 0.",
        33: "Total = Mem + CPU + Vol + Egress + Obj. Mem = 1 GB x $0.00000386 x 18,000s. CPU = 1 vCPU x $0.00000772 x 18,000s. Vol = 0.5 GB x $0.00000006 x 2,592,000s.",
        34: "Infrastructure total = KB total + Chatbot + API Gateway + Postgres + Config + Health + Redis.",
        36: "1 conversation = 5 turns.",
        37: "1 turn = 1 user input + 1 AI output.",
        38: "Avg messages / conversation = 5.",
        39: "Avg conversation time = 5 min = 300s.",
        43: "Raw MB = token M x 2.8. Chars M = token M x 2.8. Files ~= token M x 170. Words M = token M x 0.4. pgvector MB = Raw MB x 6.",
        44: "User tokens / conversation = 50 x 5 = 250.",
        45: "1st input = system prompt tokens + 1,500 tool tokens.",
        46: "Follow-up tool tokens = 1,500 x 4 = 6,000. INR subscription = USD subscription x USD-INR.",
        47: "History tokens = 350 + 2x350 + 3x350 + 4x350 = 3,500. Cost/customer = infrastructure total. INR cost = USD cost x USD-INR.",
        48: "Input tokens/conversation = user tokens + 1st input + follow-up tool tokens + history tokens. Profit/customer = subscription - cost/customer.",
        49: "Avg input tokens/turn = input tokens/conversation / 5. Customers = editable input.",
        50: "Input token cost = $0.10/M x monthly input tokens / 1,000,000. Total profit = customers x profit/customer.",
        51: "Output token cost = $0.40/M x monthly output tokens / 1,000,000. INR = USD x USD-INR.",
        52: "Cache token cost = $0.01/M x cached system-prompt tokens / 1,000,000.",
        53: "Output tokens/conversation = (response tokens/turn + tool output tokens/turn) x 5. Total LLM token cost = input + output + cache.",
        54: "Messages/conversation = 5.",
        55: "Monthly token budget = selected dropdown value.",
        56: "Avg tokens/conversation = input tokens/conversation + output tokens/conversation. Conversation hours = total seconds / 3,600.",
        57: "Conversations/month = monthly token budget / avg tokens per conversation. Storage cost = hours x $1 x system prompt tokens / 1,000,000.",
        58: "Monthly input tokens = conversations/month x input tokens/conversation. Grand LLM = storage cost + token cost.",
        59: "Monthly output tokens = conversations/month x output tokens/conversation.",
        60: "AI output messages/month = conversations/month x 5.",
        61: "Seconds/conversation = 5 min x 60 = 300s.",
        62: "Total seconds = 300s x conversations/month. Total hours = seconds / 3,600.",
        63: "Avg tokens/turn = avg input tokens/turn + response tokens/turn + tool output tokens/turn.",
        65: "Cached prompt tokens = system prompt tokens.",
        66: "Cached turns = conversations/month x 4.",
        67: "Cached token usage = cached turns x system prompt tokens.",
        70: "Credits = token budget / tokens per credit.",
        71: "1 credit = 10,000 conversation tokens.",
        72: "Total credits = monthly token budget / 10,000.",
    }
...
function updateDerivations() {{
  const serviceRow = (r, seconds) => {{
    ...
    const cpuCount = num(`E${{r}}`), cpuRate = num("F10"), cpu = cpuRate * cpuCount * seconds;
    const volGb = num(`G${{r}}`), volRate = num("H10"), vol = volRate * volGb * 2592000;
    const egressGb = num(`I${{r}}`), egressRate = num("J10"), egress = egressRate * egressGb;
  ...
  const setCostBreakdown = (r, seconds) => {{
    const memGb = num(`C${{r}}`), memRate = num("D10"), mem = memRate * seconds * memGb;
    const cpuCount = num(`E${{r}}`), cpuRate = num("F10"), cpu = cpuRate * cpuCount * seconds;
    const volGb = num(`G${{r}}`), volRate = num("H10"), vol = volRate * volGb * 2592000;
    const egressGb = num(`I${{r}}`), egressRate = num("J10"), egress = egressRate * egressGb;
    const objGb = num(`K${{r}}`), objRate = num("L10"), obj = objRate * objGb;
    setDerivation(`D${{r}}`, "Memory cost = memory rate x seconds used x GB memory", `${{usd(memRate)}} x ${{compact(seconds)}} x ${{memGb}} = ${{usd(mem)}}`);
    setDerivation(`F${{r}}`, "CPU cost = CPU rate x vCPU count x seconds used", `${{usd(cpuRate)}} x ${{cpuCount}} x ${{compact(seconds)}} = ${{usd(cpu)}}`);
    setDerivation(`H${{r}}`, "Volumes cost = volume rate x stored GB x month seconds", `${{usd(volRate)}} x ${{volGb}} x 2,592,000 = ${{usd(vol)}}`);
    setDerivation(`J${{r}}`, "Egress cost = egress rate x egress GB", `${{usd(egressRate)}} x ${{egressGb}} = ${{usd(egress)}}`);
    setDerivation(`L${{r}}`, "Object storage cost = object storage rate x stored GB", `${{usd(objRate)}} x ${{objGb}} = ${{usd(obj)}}`);
  }};
  const setServiceGroup = (r, seconds) => {{
    setCostBreakdown(r, seconds);
    setService(`O${{r}}`, r, seconds);
  }};
  for (const r of [12,13,14,15]) setServiceGroup(r, 18000);
  setDerivation("M12", "LLM Tokens (M) = ingestion token calculation", `${{fixed(num("J43"), 2)}} = ${{fixed(num("M12"), 2)}}`);
  setDerivation("N12", "Embedding cost = LLM tokens (M) x embedding price per M", `${{fixed(num("M12"), 2)}} x ${{usd(num("N10"))}} = ${{usd(num("N12"))}}`);
  ...
  setServiceGroup(18, num("B62"));
  setDerivation("N18", "LLM API cost = grand total LLM cost", `${{usd(num("G58"))}} = ${{usd(num("N18"))}}`);
  ...
  setServiceGroup(24, num("B23"));
  setServiceGroup(27, 18000);
  setCostBreakdown(30, 435000);
  setService("O30", 30, 435000);
  setServiceGroup(33, 18000);
  ...
  setDerivation("L43", "Characters (M) = token M x 2.8", `${{num("J43")}} x 2.8 = ${{fixed(num("L43"), 2)}}`);
  setDerivation("M43", "Approx files = token M x 170", `${{num("J43")}} x 170 = ${{fixed(num("M43"), 2)}}`);
  setDerivation("N43", "Words (M) = token M x 0.4", `${{num("J43")}} x 0.4 = ${{fixed(num("N43"), 2)}}`);
  setDerivation("O43", "pgvector MB = raw text MB x 6", `${{fixed(num("K43"), 2)}} x 6 = ${{fixed(num("O43"), 2)}}`);
  ...
  setDerivation("C62", "Total conversation time in hours = total seconds / 3,600", `${{compact(num("B62"))}} / 3,600 = ${{fixed(num("C62"), 2)}}`);
  ...
  setDerivation("F56", "Total conversation time in hours = total conversation hours", `${{fixed(num("C62"), 2)}} = ${{fixed(num("F56"), 2)}}`);
  setDerivation("G57", "Storage cost = hours x cache storage price x system prompt tokens / 1,000,000", `${{fixed(num("F56"), 2)}} x ${{usd(num("F55"))}} x ${{compact(num("B65"))}} / 1,000,000 = ${{usd(num("G57"))}}`);
  setDerivation("G58", "Grand total LLM cost = cache storage cost + token cost", `${{usd(num("G57"))}} + ${{usd(num("G53"))}} = ${{usd(num("G58"))}}`);
  setDerivation("M47", "Cost per customer = infrastructure total", `${{usd(num("O34"))}} = ${{usd(num("M47"))}}`);
  setDerivation("N46", "Subscription price INR = subscription USD x USD-INR", `${{usd(num("M46"))}} x ${{fixed(num("M51"), 2)}} = ${{fixed(num("N46"), 2)}}`);
  setDerivation("N47", "Cost per customer INR = cost per customer USD x USD-INR", `${{usd(num("M47"))}} x ${{fixed(num("M51"), 2)}} = ${{fixed(num("N47"), 2)}}`);
  setDerivation("M48", "Profit per customer = subscription USD - cost per customer USD", `${{usd(num("M46"))}} - ${{usd(num("M47"))}} = ${{usd(num("M48"))}}`);
  setDerivation("N48", "Profit per customer INR = profit per customer USD x USD-INR", `${{usd(num("M48"))}} x ${{fixed(num("M51"), 2)}} = ${{fixed(num("N48"), 2)}}`);
  setDerivation("M50", "Total profit = customers x profit per customer", `${{compact(num("M49"))}} x ${{usd(num("M48"))}} = ${{usd(num("M50"))}}`);
  setDerivation("N50", "Total profit INR = total profit USD x USD-INR", `${{usd(num("M50"))}} x ${{fixed(num("M51"), 2)}} = ${{fixed(num("N50"), 2)}}`);
