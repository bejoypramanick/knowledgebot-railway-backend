...
class CorrelationIDMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        logger.info(f"🔍 [{correlation_id}] {request.method} {request.url.path} - Request received")
        ...
        logger.info(f"🔍 [{correlation_id}] {request.method} {request.url.path} - Response sent (Status: {response.status_code})")
