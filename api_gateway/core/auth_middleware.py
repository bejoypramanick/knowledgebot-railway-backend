...
class SessionAuthMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        # Session UUID comes from httpOnly cookie (set by /set-current or after first message)
        # First message has NO session_id - it's generated by chatbot-orchestration and returned to frontend
        # Subsequent messages have UUID in cookie (set by /set-current)
        ...
        if session_uuid:
            ...
            if numeric_session_id:
                ...
                logger.debug(f"⚠️ Could not resolve session UUID {session_uuid}")
