...
class StreamingService:
    ...
    def _extract_model_message_type(self, response_text: str) -> tuple[str | None, str]:
        ...
        match = re.match(
            r"^\s*MESSAGE_TYPE:\s*(PURE_GREETING|NON_GREETING)\s*\n?",
            response_text,
            flags=re.IGNORECASE,
        )
        if not match:
    ...
    def _has_citation_markers(self, response_text: str) -> bool:
        ...
        if re.search(r"\[\s*\d+(?:\s*,\s*\d+)+\s*\]", response_text):
            return True
        if re.search(
            r"<a[^>]+>\\s*\\[\\d+\\]\\s*</a>", response_text, flags=re.IGNORECASE
        ):
            return True
        return False
    ...
    def _convert_db_messages_to_pydantic_ai(
        ...
        for msg in db_messages:
            try:
                ...
                if not content and not msg.get("content"):
                    logger.warning(
                        f"⚠️ WARNING: Message has no content - checking DB structure"
                    )
                    logger.warning(f"⚠️ Available fields: {list(msg.keys())}")

                if msg.get("role") == "user":
                    # Convert user message
                    user_msg = ModelRequest(parts=[UserPromptPart(content=content)])
                    pydantic_messages.append(user_msg)
                    logger.debug(f"🔄 Converted user message: {content[:50]}...")

                elif msg.get("role") == "assistant":
                    # Convert assistant message
                    assistant_msg = ModelResponse(parts=[TextPart(content=content)])
        ...
        logger.info(
            f"✅ Converted {len(pydantic_messages)} messages to Pydantic AI format"
        )
    ...
    def _detect_agent_request(self, message: str) -> bool:
        ...
        for keyword in agent_keywords:
            if keyword in message_lower:
                logger.info(
                    f"🎯 Detected agent request keyword: '{keyword}' in message: '{message[:50]}...'"
                )
                return True

        return False

    def _prune_message_history_safe(
        self, messages: List[Any], max_messages: int = 50
    ) -> List[Any]:
        """
        ...
        has_system_prompt = (
            len(messages) > 0
            and hasattr(messages[0], "parts")
            and any(isinstance(part, SystemPromptPart) for part in messages[0].parts)
        )
        ...
        if has_system_prompt:
            ...
            logger.info(
                f"🔄 Pruned message history (protected system prompt at index 0)"
            )
    ...
    async def stream_agent_response(
        ...
        try:
            ...
            enable_s3_upload = (
                os.getenv("ENABLE_RAG_S3_UPLOAD", "true").lower() == "true"
            )

            if enable_s3_upload:
                logger.info(
                    "📁 Agent S3 upload is ENABLED - uploading agent request data..."
                )
                try:
                    from ..tools.knowledge_tools import _upload_agent_request_to_s3

                    # Get conversation history for context
                    chat_history = await session_state_manager.get_chat_history(
                        session_id
                    )

                    agent_request_download_url = await _upload_agent_request_to_s3(
                        session_id, message, chat_history
                    )
                    if agent_request_download_url:
                        logger.info(
                            f"📁 ✅ Agent request uploaded to S3: {agent_request_download_url}"
                        )
                    else:
                        logger.warning(
                            "📁 ⚠️ Failed to upload agent request to S3 (returned None)"
                        )
                ...
                logger.info(
                    "📁 Agent S3 upload is DISABLED (ENABLE_RAG_S3_UPLOAD=false or not set)"
                )
            ...
            if chat_history:
                logger.info(
                    f"📚 [CHAT_HISTORY] session={session_id[:16]} messages={len(chat_history)}"
                )
            ...
            if not cache_name:
                raise RuntimeError(
                    f"Gemini cache is required but unavailable for session {session_id}"
                )
            ...
            if user_wants_agent:
                logger.info(
                    f"🧑 User explicitly requesting human agent - assigning before AI responds"
                )
                    ...
                    if session_id:
                        ...
                        async with httpx.AsyncClient(timeout=10.0) as client:
                            response = await client.post(
                                endpoint_url, json={"session_id": session_id}
                            )
                            ...
                            if response.status_code == 200:
                                ...
                                logger.info(
                                    f"✅ Agent {assigned_agent} (ID: {assigned_agent_id}) assigned before AI response"
                                )
                                ...
                                from shared.sqlalchemy_db import (
                                ...
                                async with get_db() as verify_db:
                                    ...
                                    verify_result = await verify_db.execute(
                                        sql_text(verify_query),
                                        {"session_id": session_id},
                                    )
                                    verify_row = verify_result.mappings().first()
                                    if verify_row:
                                        logger.info(
                                            f"Verified agent assignment in database: {verify_row['agent_email']} (ID: {verify_row['agent_id']})"
                                        )
                                    else:
                                        logger.warning(
                                            f"Agent assignment not found in database yet for session {session_id}"
                                        )
                                ...
                                logger.info(
                                    f"Cached agent assignment (DB 4): {session_id} -> {assigned_agent_id}"
                                )
                                ...
                                redis_messages = await chat_store.get_messages(
                                    session_id
                                )
                                ...
                                if redis_session:
                                    ...
                                    user_display_id = await self._get_user_display_id(
                                        session_id
                                    )
                                    ...
                                    session_event = {
                                        ...
                                        "data": {
                                            ...
                                            "status": redis_session.get(
                                                "archive_status", "active"
                                            ),
                                            "last_message_at": redis_session.get(
                                                "last_activity_at",
                                                datetime.utcnow().isoformat(),
                                            ),
                                            "created_at": redis_session.get(
                                                "started_at"
                                            ),
                                            "assigned_agent": assigned_agent,
                                    ...
                                    result = await broadcast_event_to_agent(
                                        assigned_agent_id, session_event
                                    )
                                    logger.info(
                                        f"📤 Broadcasted session_update to agent {assigned_agent} (ID: {assigned_agent_id}) on channel agent:events:{assigned_agent_id}"
                                    )
                                    ...
                                    broadcast_result = (
                                        await broadcast_event_to_all_agents(
                                            session_event
                                        )
                                    )
                                    logger.info(
                                        f"📢 Broadcasted session_update to ALL admins on channel agent:events:broadcast"
                                    )
                                    logger.info(
                                        f"📢 Broadcast result: {broadcast_result}"
                                    )
                                    logger.info(
                                        f"📤 Session data includes {len(messages)} messages"
                                    )
                                ...
                                customer_broadcast_result = (
                                    await broadcast_event_to_session(
                                        session_id, chat_assigned_event
                                    )
                                )
                                logger.info(
                                    f"📤 Sent chat_assigned event to customer on channel customer:events:{session_id}"
                                )
                                logger.info(
                                    f"📤 Customer broadcast result: {customer_broadcast_result}"
                                )
                                ...
                                logger.warning(
                                    f"⚠️ No agents available - letting AI respond"
                                )
                                # Fall through to normal AI response
                            else:
                                logger.error(
                                    f"❌ Agent assignment failed: {response.status_code}"
                                )
                                # Fall through to normal AI response
            ...
            logger.info(
                f"Checking if human agent is assigned to session {session_id}..."
            )
            try:
                from shared.redis_agent_cache import (
                    get_assigned_agent,
                    set_assigned_agent as cache_set_agent,
                )
                    ...
                    try:
                        ...
                        async with get_db_session() as db_session:
                            ...
                            if row:
                                ...
                                logger.info(
                                    f"Found agent in database: {assigned_agent_email} (ID: {assigned_agent_id})"
                                )
                ...
                if assigned_agent_id:
                    assigned_agent_id = (
                        ...
                        if isinstance(assigned_agent_id, bytes)
                        else assigned_agent_id
                    )
                    logger.info(
                        f"👤 Human agent (ID: {assigned_agent_id}) is assigned to session {session_id}"
                    )
                    ...
                    try:
                        ...
                        logger.info(
                            f"📤 Notified agent (ID: {assigned_agent_id}) about new customer message"
                        )
                    except Exception as e:
                    ...
                    logger.info(
                        f"✅ Customer message processed and forwarded to agent (ID: {assigned_agent_id})"
                    )
                    return

            except Exception as e:
                logger.warning(
                    f"⚠️ Error checking agent assignment: {e} - proceeding with AI response"
                )
            ...
            try:
                ...
                logger.info(
                    f"📤 Broadcasted user message to admins on agent:events:broadcast"
                )
            except Exception as broadcast_error:
                logger.error(
                    f"❌ Failed to broadcast user message to admins: {broadcast_error}"
                )
            ...
            try:
                ...
                try:
                    ...
                    logger.warning(
                        f"Could not fetch response_policy: {e}, using default 0.5"
                    )
                ...
                if ENABLE_EXTENDED_THINKING:
                    ...
                    model_settings_kwargs["google_thinking_config"] = (
                        ThinkingConfigDict(include_thoughts=True)
                    )
                ...
                logger.info(
                    f"Agent.iter() call: message='{message[:80]}...', "
                ...
                logger.info(
                    f"⏳ Checking token rate limit (estimated: {estimated_tokens} tokens)..."
                )
                ...
                async def _run_agent_once(
                    active_model_settings, override_message=None, override_history=None
                ):
                        ...
                        try:
                            for msg in (messages or [])[-4:]:
                                ...
                                for idx, part in enumerate(parts[:4]):
                                    msg_summary_parts.append(
                                        f"p{idx}={type(part).__name__}"
                                    )
                                    tool_name = getattr(part, "tool_name", None)
                                    if tool_name:
                                    ...
                                    if isinstance(content, str) and content:
                                        msg_summary_parts.append(
                                            f"text='{content.replace(chr(10), ' ')[:120]}'"
                                        )
                                    args = getattr(part, "args", None)
                                    if args is not None:
                                        msg_summary_parts.append(
                            ...
                            summaries.append(
                                f"message_history_summary_error={history_error}"
                            )
                        return summaries
                    ...
                    try:
                        ...
                        active_history = (
                            ...
                            else pydantic_messages
                        )
                        use_message_history = active_model_settings.get(
                            "google_cached_content"
                        ) is not None or bool(active_history)
                        ...
                        if use_message_history:
                            async with agent.iter(
                                ...
                                async for event in active_run:
                                    ...
                                    try:
                                        run_ctx = getattr(
                                            active_run, "_graph_run", None
                                        )
                                        state = getattr(run_ctx, "state", None)
                                        message_history = getattr(
                                            state, "message_history", None
                                        )
                                        if message_history:
                                            last_message_history_summary = (
                                                _summarize_message_history(
                                                    message_history
                                                )
                                            )
                                    except Exception:
                ...
                logger.info(
                    "🔍 Agent iteration completed, extracting response from all_messages()..."
                )
                try:
                    ...
                    if enable_s3_upload:
                        logger.info(
                            "📁 Agent S3 upload is ENABLED - attempting upload..."
                        )
                        try:
                            from ..tools.knowledge_tools import (
                                _upload_agent_response_to_s3,
                            )

                            agent_s3_download_url = await _upload_agent_response_to_s3(
                                session_id, all_messages, run
                            )
                            if agent_s3_download_url:
                                logger.info(
                                    f"📁 ✅ Agent response uploaded to S3: {agent_s3_download_url}"
                                )
                            else:
                                logger.warning(
                                    "📁 ⚠️ Failed to upload agent response to S3 (returned None)"
                                )
                        except Exception as s3_error:
                    ...
                    if not all_messages:
                        ...
                        logger.debug(
                            f"all_messages received: count={len(all_messages)}"
                        )
                    ...
                    for i, msg in enumerate(all_messages):
                        ...
                        if hasattr(msg, "parts"):
                            for j, part in enumerate(msg.parts):
                                ...
                                if isinstance(
                                    part, (BuiltinToolCallPart, ToolCallPart)
                                ):
                                    ...
                                    if isinstance(tool_args, dict):
                                        logger.info(
                                            f"   🔍 [TOOL_QUERY] args_keys={list(tool_args.keys())[:20]}"
                                        )
                                    else:
                                        logger.info(
                                            f"   🔍 [TOOL_QUERY] args_type={type(tool_args).__name__}"
                                        )

                                # Detect & log tool RETURNS
                                elif isinstance(
                                    part, (BuiltinToolReturnPart, ToolReturnPart)
                                ):
                                    tool_name = getattr(part, "tool_name", "unknown")
                                    ...
                                    logger.info(
                                        f"📚 [TOOL_RETURN] name={tool_name} content_chars={len(str(content))}"
                                    )

                                # Log text content (model's final answer)
                                elif part_type == "TextPart" and hasattr(
                                    part, "content"
                                ):
                                    content = getattr(part, "content", "")
                                    logger.debug(
                                        f"   📝 {part_type}: chars={len(str(content))}"
                                    )

                    logger.debug(
                        f"tools_called={len(tool_calls_made)} unique={len(set(tool_calls_made))}"
                    )
                    if len(tool_calls_made) == 0 and len(pydantic_messages) > 0:
                        logger.warning(
                            "⚠️ Follow-up message but no tools were called (expected KB search)"
                        )
                    ...
                    if True:
                        ...
                        for i, msg in enumerate(all_messages):
                            ...
                            if hasattr(msg, "parts"):
                                ...
                                logger.debug(
                                    f"Has {total_parts} parts ({len(text_parts)} TextParts)"
                                )

                                for j, part in enumerate(msg.parts):
                                    part_type = type(part).__name__
                                    logger.debug(
                                        f"     Part {j}/{total_parts - 1}: {part_type}"
                                    )
                                    ...
                                    thinking_attrs = [
                                        ...
                                        if "think" in attr.lower()
                                    ]
                                    if thinking_attrs:
                                        logger.debug(
                                            "     Thinking-related attributes present"
                                        )
                                    ...
                                    if hasattr(part, "content"):
                                        content = getattr(part, "content", "")
                                        logger.debug(
                                            f"     Content type={type(content).__name__} len={len(str(content))}"
                                        )
                                    ...
                                    if part_type == "ThinkingPart":
                                        ...
                                        logger.debug(
                                            "ThinkingPart present (content suppressed)"
                                        )
                                    ...
                                    elif hasattr(part, "thinking") or hasattr(
                                        part, "thoughts"
                                    ):
                                        logger.debug(
                                            "Alternative thinking attribute found (suppressed)"
                                        )
                                        thinking_attr = getattr(
                                            part, "thinking", None
                                        ) or getattr(part, "thoughts", None)
                                        if thinking_attr:
                                            logger.info(
                                                "🧠 ALTERNATIVE THINKING CONTENT present (suppressed)"
                                            )
                    ...
                    if tool_call_count == 0 and message.strip():
                        ...
                        is_greeting = any(
                            g in message.lower() for g in greeting_patterns
                        )
                        ...
                        if not is_greeting:
                            logger.error(
                                f"TOOL CALL REQUIREMENT NOT MET: query='{message[:80]}', tools=0"
                            )
                            logger.error(f"🚨 Response was: {full_response[:100]}...")
                            logger.error(
                                "🚨 Model returned zero tool calls; final enforcement will use model-reported MESSAGE_TYPE"
                            )

                except Exception as result_error:
                    logger.error(
                        f"❌ Error extracting results: {result_error}", exc_info=True
                    )
            ...
            model_message_type, full_response = self._extract_model_message_type(
                full_response
            )
            ...
            if (
                tool_call_count == 0
                and message.strip()
                and effective_message_type == "NON_GREETING"
            ):
                logger.warning(
                    "🔁 No retrieval tool call detected for effective NON_GREETING message; retrying once with forced tool calling"
                )
                ...
                if retry_run is not None:
                    ...
                    if retry_result is None:
                        raise RuntimeError(
                            "Forced tool retry completed without a final result"
                        )
                    ...
                    model_message_type, full_response = (
                        self._extract_model_message_type(full_response)
                    )
                    effective_message_type = model_message_type or "NON_GREETING"
                    logger.info(
                ...
                if tool_call_count == 0 and effective_message_type == "NON_GREETING":
                    logger.error(
                        "❌ Forced retrieval retry still produced zero tool calls; returning no-answer response"
                    )
            ...
            if full_response:
                logger.info(
                    "📤 Streaming final response in chunks (after enforcement check)..."
                )
                ...
                try:
                    logger.info(
                        "📎 [CITATION_POST] No structured citation payload available for pgvector tool"
                    )

                    if citation_urls:
                        logger.info(
                            f"📎 [CITATION_POST] Found {len(citation_urls)} citation URLs: {citation_urls}"
                        )
                        ...
                        for i, url in enumerate(citation_urls, 1):
                            ...
                            if plain_marker in full_response:
                                full_response = full_response.replace(
                                    plain_marker, clickable_link
                                )
                                markers_replaced += 1
                                logger.info(
                                    f"📎 [CITATION_POST] Replaced {plain_marker} → clickable link to {url}"
                                )
                        ...
                        if markers_replaced == 0:
                            logger.warning(
                                f"📎 [CITATION_POST] No [N] markers in response — appending sources section"
                            )
                            sources_html = (
                                '<p class="citation-sources"><strong>Sources:</strong> '
                            )
                            source_links = []
                            for i, url in enumerate(citation_urls, 1):
                                source_links.append(
                            ...
                            logger.info(
                                f"📎 [CITATION_POST] Appended {len(citation_urls)} source links as fallback"
                            )
                        else:
                            logger.info(
                                f"📎 [CITATION_POST] Replaced {markers_replaced} inline markers"
                            )
                    else:
                        logger.info(
                            "📎 [CITATION_POST] No structured citations found in response"
                        )
                except Exception as citation_error:
                    logger.warning(
                        f"📎 [CITATION_POST] Citation processing failed (non-fatal): {citation_error}"
                    )
                ...
                try:
                    ...
                    had_citations_before = self._has_citation_markers(
                        before_enforcement or ""
                    )
                    rag_source_urls = session_state_manager.get_last_citation_urls(
                        session_id
                    )
                    rag_sources_count = len(rag_source_urls)
                    grounding_received_from_rag = bool(rag_source_urls)
                    session_tools_used = session_state_manager.get_tools_used(
                        session_id
                    )
                    ...
                    full_response = self._enforce_grounded_citation_policy(
                        model_message_type,
                        full_response,
                        grounding_received_from_rag=bool(
                            session_state_manager.get_last_citation_urls(session_id)
                        ),
                    )
                ...
                try:
                    ...
                    logger.warning(
                        f"📎 [CITATION_POST] Inline link rendering failed (non-fatal): {link_err}"
                    )
                ...
                if agent_request_download_url:
                    download_links.append(
                        f"[Download Agent Request]({agent_request_download_url})"
                    )
                    logger.info(
                        f"📁 ✅ Added agent request download link: {agent_request_download_url}"
                    )

                # Add agent response link
                if agent_s3_download_url:
                    download_links.append(
                        f"[Download Agent Response]({agent_s3_download_url})"
                    )
                    logger.info(
                ...
                if download_links:
                    ...
                    logger.info(
                        f"📁 ✅ Full response now includes agent debug section (total length: {len(full_response)} chars)"
                    )
                else:
                    logger.info(
                        "📁 ❌ No agent download links added - both URLs are None"
                    )
                ...
                try:
                    ...
                    broadcast_result = await broadcast_event_to_all_agents(
                        ai_response_event
                    )
                    logger.info(
                        f"📤 Broadcasted bot response to admins on agent:events:broadcast"
                    )
                    logger.info(f"📤 Broadcast result: {broadcast_result}")
                except Exception as broadcast_error:
                    logger.error(
                        f"❌ Failed to broadcast bot response to admins: {broadcast_error}"
                    )
                ...
                for idx, chunk in enumerate(chunks, 1):
                    if chunk.strip():  # Only stream non-empty chunks
                        logger.debug(
                            f"Streaming chunk {idx}/{len(chunks)} chars={len(chunk)}"
                        )
                        response_data = {
                            "type": "chunk",
                            "content": chunk,
                ...
                logger.info(
                    f"📦 Streamed final response in {len(chunks)} chunks ({len(full_response)} chars total)"
                )
            ...
            if full_response.strip():
                ...
                try:
                    ...
                    logger.info(
                        f"✅ Assistant response saved to database ({len(full_response)} chars)"
                    )
                except Exception as db_error:
            ...
            customer_response_event = {
                ...
                "content": full_response,
                "chunk_count": chunk_count,
                "debug_attachment_count": len(download_links),
                "tool_calls": tool_call_count,
                "timestamp": int(time.time()),
                "grounding_sources": "pgvector",
                "response_format": "HTML with citations",
            ...
            try:
                customer_result = await broadcast_event_to_session(
                    session_id, customer_response_event
                )
                logger.info(
                    f"✅ Posted AI response to customer channel: customer:events:{session_id}"
                )
            ...
            try:
                ...
                if assigned_agent_id:
                    ...
                    admin_result = await broadcast_event_to_agent(
                        assigned_agent_id, admin_response_event
                    )
                    logger.info(
                        f"✅ Posted AI response to agent channel: agent:events:{assigned_agent_id}"
                    )
                    ...
                    logger.info(
                        f"✅ Posted AI response to admin broadcast channel: agent:events:broadcast"
                    )
                    logger.info(f"   Broadcast result: {broadcast_result}")
            except Exception as e:
            ...
            try:
                ...
                system_prompt_text = (
                    agent_manager.get_cached_system_prompt(session_id) or ""
                )
            ...
            logger.error(
                f"❌ Critical error in stream_agent_response: {e}", exc_info=True
            )
    ...
    async def _track_token_usage(
        ...
        try:
            ...
            model_name = os.getenv(
                "GEMINI_MODEL_NAME", os.getenv("CHATBOT_MODEL", "gemini-2.5-flash-lite")
            )
            ...
            if success:
                logger.info(
                    f"✅ Tracked {total_tokens} tokens for session {session_id[:8]}... ({token_source})"
                )
            else:
            ...
            asyncio.create_task(
                self._update_message_and_session_usage(
                    session_id=session_id,
        ...
        for attempt in range(3):
            try:
                ...
                async with get_db_session() as db:
                    ...
                    logger.info(
                        f"✅ [USAGE_UPDATE] Updated message & session usage for {session_id[:8]}..."
                    )
                    logger.info(
                        f"   User msg: {user_char_count} chars, {user_word_count} words, msg_tokens={user_message_tokens}, prompt_tokens={input_tokens} (updated={user_updated})"
                    )
                    logger.info(
                    ...
                    logger.info(
                        f"   Session totals += {total_char} chars, {total_word} words, {total_token} tokens, {total_msg_tokens} msg_tokens"
                    )
                    return

            except Exception as e:
                logger.warning(
                    f"⚠️ [USAGE_UPDATE] Attempt {attempt + 1}/3 failed for {session_id[:8]}...: {e}"
                )
    ...
    async def _save_agent_run_steps(self, session_id: str, run):
        ...
        try:
            ...
            for msg in all_messages:
                ...
                step_type = (
                    "model_request" if msg_type == "ModelRequest" else "model_response"
                )
            ...
            logger.info(
                f"✅ [RUN_STEPS] Saved {len(steps)} agent run steps for session {session_id[:8]}..."
            )
            for s in steps:
                logger.info(
                    f"   Step {s['step_number']}: {s['step_type']}/{s['part_type']} - {s.get('token_count', 0)} tokens, {s['char_count']} chars"
                )
