"""
Correlation ID Middleware for Chatbot Orchestration Service.
...
class CorrelationIDMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        logger.info(f"🔍 [{correlation_id}] {request.method} {request.url.path} - Chatbot request received")
        ...
        logger.info(f"🔍 [{correlation_id}] {request.method} {request.url.path} - Chatbot response sent (Status: {response.status_code})")
