...
async def create_chat_session(session_identifier: str):
    """
    Create a chat session and return its numeric database ID.

    Called during session initialization to register a new session and get
    ...
    """
    try:
        # Use SessionPersistenceDAO to create and register the session
        session_dao = SessionPersistenceDAO()
        numeric_id = await session_dao.get_or_create_session(session_identifier)

        logger.info(f"✅ Created session with numeric ID {numeric_id}")
