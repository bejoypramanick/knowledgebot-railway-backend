...
async def get_or_create_session_numeric_id(uuid_session_id: str):
    """
    BOOTSTRAP ENDPOINT: Convert UUID → numeric database ID during session creation.

    Called ONCE by frontend during createChatSession() to register a session and get
    the numeric ID from chat_sessions table for all subsequent operations.

    After this call, ONLY the numeric ID is used. UUID is for message identification only.

    Args:
        uuid_session_id: UUID-style session ID generated by frontend (e.g., session_1772521413864_5o610dbd1)

    Returns:
        {"success": True, "data": {"id": <numeric_id>}}
