...
# Initialize DAO (assumes db.railway_db is available or will be checked)
def get_file_dao():
    return FileDAO(db.railway_db)
...
async def get_or_create_user(email: str) -> str:
    ...
    try:
        dao = get_file_dao()
        user_email = await dao.get_user_by_email(email)
...
async def check_duplicate_file(sha256_hash: str, original_filename: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        existing = await dao.find_duplicate_by_hash(sha256_hash)
        if existing:
            return {
                "id": str(existing['id']),
        ...
        existing_by_name = await dao.find_duplicate_by_name(original_filename)
        if existing_by_name:
            return {
...
async def delete_existing_file_record(db_id: str):
    """Delete an existing file record from database."""
    try:
        dao = get_file_dao()
        await dao.delete_file_record(db_id)
        logger.info(f"Deleted old file record from database: {db_id}")
    except Exception as e:
        logger.error(f"Error deleting existing file record: {e}")

async def record_metadata(user_id: str, original_filename: str, file_display_name: str, 
    ...
    try:
        ...
        db_record_id = await dao.insert_file_record(record_data)
        logger.info(f"✅ [DB] Record created with ID: {db_record_id} (version {version})")
        
        # Log metric
        await dao.record_metric({
            'type': 'file_upload',
