# Only proceed with full service setup if imports succeeded
if IMPORT_SUCCESS:
    ...
    # Database will be initialized lazily when first needed (see get_railway_db function)
    logger.info("🔄 Database will be initialized lazily for token tracking")
            ...
            try:
                ...
                def extract_original_filename(display_name: str) -> str:
                    ...
                    if ' | ' in display_name:
                        ...
                        # No display_name IS the original filename
                        return display_name
    ...
    # If imports failed, add a degraded health endpoint
    @app.get("/health")
    async def degraded_health_check():
        """Health check when service is in degraded mode."""
        return {
            "status": "degraded",
            "service": "chatbot_orchestration",
            "message": "Service started but some features may be unavailable due to import failures",
            "timestamp": datetime.utcnow().isoformat(),
