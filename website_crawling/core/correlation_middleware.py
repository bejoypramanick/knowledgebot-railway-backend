"""
Correlation ID Middleware for Website Crawling Service.
...
class CorrelationIDMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        logger.info(f"🔍 [{correlation_id}] {request.method} {request.url.path} - Website crawling request received")
        ...
        logger.info(f"🔍 [{correlation_id}] {request.method} {request.url.path} - Website crawling response sent (Status: {response.status_code})")
