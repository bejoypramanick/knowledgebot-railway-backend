...
def log_query(query: str, params: Optional[List[Any]] = None, result: Optional[Any] = None, operation: str = "EXECUTE"):
    ...
    logger.info(f"🔍 {correlation_prefix}DB Query [{operation}]: {query.strip()}")
    if params:
        logger.info(f"🔍 {correlation_prefix}DB Parameters: {params}")
    if result is not None:
        if isinstance(result, str):
            logger.info(f"✅ {correlation_prefix}DB Result: {result}")
        elif hasattr(result, '__len__'):
            logger.info(f"✅ {correlation_prefix}DB Result: {len(result)} rows affected/returned")
        else:
            logger.info(f"✅ {correlation_prefix}DB Result: {result}")
...
async def execute_with_logging(conn: asyncpg.Connection, query: str, *params, operation: str = "EXECUTE"):
    ...
    logger.info(f"🔍 {correlation_prefix}DEBUG: execute_with_logging called for operation={operation}")
    logger.info(f"🔍 {correlation_prefix}DEBUG: Query={query.strip()}")
    logger.info(f"🔍 {correlation_prefix}DEBUG: Params={list(params) if params else None}")
    
    log_query(query, list(params) if params else None, operation=operation)
    logger.info(f"🔍 {correlation_prefix}DEBUG: About to execute query")
    result = await conn.execute(query, *params)
    logger.info(f"🔍 {correlation_prefix}DEBUG: Query executed, result={result}")
    log_query(query, list(params) if params else None, result, operation)
    logger.info(f"🔍 {correlation_prefix}DEBUG: execute_with_logging completed")
    return result
