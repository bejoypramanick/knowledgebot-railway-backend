"""
Correlation ID Middleware for Configuration Service.
...
class CorrelationIDMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        logger.info(f"🔍 [{correlation_id}] {request.method} {request.url.path} - Configuration request received")
        ...
        logger.info(f"🔍 [{correlation_id}] {request.method} {request.url.path} - Configuration response sent (Status: {response.status_code})")
