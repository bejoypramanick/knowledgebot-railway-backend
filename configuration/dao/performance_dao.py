...
class PerformanceDAO:
    ...
    async def get_recent_interactions(self) -> int:
        ...
        return await self.conn.fetchval("SELECT COUNT(*) FROM chat_messages WHERE role = 'user' AND created_at >= NOW() - INTERVAL '30 days'") or 0

    async def get_previous_interactions(self) -> int:
        """Get interactions from 30-60 days ago."""
        return await self.conn.fetchval("SELECT COUNT(*) FROM chat_messages WHERE role = 'user' AND created_at >= NOW() - INTERVAL '60 days' AND created_at < NOW() - INTERVAL '30 days'") or 0
