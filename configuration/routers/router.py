...
async def get_feedback_counts(request: Request, user: dict = Depends(get_current_user)):
    ...
    Request Body:
        session_ids: List of numeric session IDs (e.g., ["269", "268", "267"])

    Returns:
        Dictionary with numeric session_id as key and {positive: count, negative: count} as value
    """
    try:
        # Get session_ids from request body
        body = await request.json()
        numeric_session_ids = body.get("session_ids", [])

        if not numeric_session_ids:
            return {"success": True, "data": {}}

        # Ensure session_ids is a list
        if isinstance(numeric_session_ids, str):
            numeric_session_ids = [sid.strip() for sid in numeric_session_ids.split(",") if sid.strip()]
        elif not isinstance(numeric_session_ids, list):
            return {"success": False, "error": "session_ids must be a list or comma-separated string"}

        if not numeric_session_ids:
            return {"success": True, "data": {}}

        logger.info(f"🔍 Getting feedback counts for {len(numeric_session_ids)} numeric session IDs")

        # Convert numeric IDs to UUID session_ids
        # Query: SELECT id, session_id FROM chat_sessions WHERE id IN (...)
        from sqlalchemy import text
        from shared.sqlalchemy_db import get_db_session
        
        placeholders = ",".join([f":id_{i}" for i in range(len(numeric_session_ids))])
        params = {f"id_{i}": int(sid) for i, sid in enumerate(numeric_session_ids)}
        
        query = f"SELECT id, session_id FROM chat_sessions WHERE id IN ({placeholders})"
        
        async with get_db_session() as session:
            result = await session.execute(text(query), params)
            rows = result.fetchall()
        
        # Build mapping: numeric_id -> uuid_session_id
        id_to_uuid = {str(row['id']): row['session_id'] for row in rows}
        uuid_session_ids = list(id_to_uuid.values())
        
        logger.info(f"🔍 Converted {len(numeric_session_ids)} numeric IDs to {len(uuid_session_ids)} UUIDs")

        # Get feedback counts using UUID session_ids
        uuid_counts = await feedback_service.get_feedback_counts_by_sessions(uuid_session_ids)
        
        # Convert back to numeric IDs for response
        numeric_counts = {}
        for numeric_id, uuid_sid in id_to_uuid.items():
            numeric_counts[numeric_id] = uuid_counts.get(uuid_sid, {'positive': 0, 'negative': 0})
        
        # Fill in missing IDs with zero counts
        for numeric_id in numeric_session_ids:
            if numeric_id not in numeric_counts:
                numeric_counts[numeric_id] = {'positive': 0, 'negative': 0}

        return {"success": True, "data": numeric_counts}
