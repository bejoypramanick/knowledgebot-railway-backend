...
async def _invalidate_kb_file_list_cache() -> None:
    ...
    try:
        ...
        logger.debug(
            f"🗑️ [KB_CACHE_INVALIDATE] Purged {deleted} KB file-list cache keys"
        )
    except Exception as cache_err:
        logger.warning(
            f"⚠️ [KB_CACHE_INVALIDATE] Failed to purge KB file-list cache: {cache_err}"
        )
...
class ScrapingDAO:
    ...
    async def update_website_status(
        self, website_id: str, status: str, error_message: str = None
    ):
        ...
        if error_message:
            ...
            logger.debug(
                f"💾 [WEB_UPDATE_START] Updating website {website_id} status to {status}"
            )
        ...
        try:
            async with get_db_session() as session:
                ...
                if result.rowcount == 0:
                    ...
                    logger.debug(
                        f"✅ [WEB_UPDATE_SUCCESS] Website {website_id} status updated to {status}"
                    )
    ...
    async def record_scraped_metadata(
        ...
        if not url_type:
            ...
            is_sitemap = (
                ...
                and (url_lower.endswith(".xml") or url_lower.endswith(".xml.gz"))
                or "sitemap" in path.lower()
                and path.lower().endswith(".xml")
            )
        ...
        try:
            ...
            async with get_db_session() as session:
                ...
                logger.info(
                    f"✅ [WEB_INSERT_SUCCESS] Website record created with ID: {result}"
                )
                logger.log_db_query(query, params, result)
                await session.commit()
            ...
            logger.error(
                f"❌ [WEB_INSERT_ERROR] Failed to record website metadata: {e}"
            )
    ...
    async def delete_website_record(self, url: str):
        ...
        try:
            ...
            async with get_db_session() as session:
                ...
                await session.commit()
                logger.info(
                    f"✅ [WEB_DELETE_SUCCESS] Website record deleted. Result: {result}"
                )
    ...
    async def check_duplicate_website(
        ...
        """
        logger.info(
            f"🔍 [WEB_DUPLICATE_CHECK] Checking for duplicate website URL: {original_url}"
        )
        ...
        try:
            ...
            async with get_db_session() as session:
                ...
                if result:
                    logger.info(
                        f"⚠️  [WEB_DUPLICATE_FOUND] Website already exists (ID: {result.id}, Status: {result.processing_status})"
                    )
                    logger.log_db_query(query, params, result)
                    return {
                        "id": result.id,
                    ...
                    logger.info(
                        f"✅ [WEB_DUPLICATE_NOT_FOUND] No active website found for URL: {original_url}"
                    )
            ...
            logger.warning(
                f"⚠️  [WEB_DUPLICATE_CHECK_ERROR] Error checking for duplicate website: {e}"
            )
            logger.log_db_query(query, params, error=e)
            return None
    ...
    async def get_admin_user_role_id(self, user_email: str = None) -> Optional[str]:
        ...
        try:
            async with get_db_session() as session:
                ...
                user_role = (
                    await session.execute(
                        text("""SELECT urm.user_role_id
                       FROM user_role_mapping urm
                       JOIN users u ON urm.user_id = u.id
                       WHERE u.email = :email AND urm.role_id = :role_id LIMIT 1"""),
                        {"email": email_to_search, "role_id": admin_role},
                    )
                ).scalar()
    ...
    async def record_child_page(
        ...
        logger.info(
            f"📄 [CHILD_PAGE_INSERT] Recording child page for parent {parent_id}"
        )
        ...
        try:
            ...
            async with get_db_session() as session:
                ...
                parent_check_query = (
                    "SELECT id FROM scraped_websites WHERE id = :parent_id"
                )
                parent_exists = (
                    await session.execute(
                        text(parent_check_query), {"parent_id": parent_id}
                    )
                ).scalar()

                if not parent_exists:
                    logger.warning(
                        f"⚠️  [CHILD_PAGE_SKIP] Parent ID {parent_id} not found, skipping child page"
                    )
                    logger.warning(f"   URL: {page_url}")
                    logger.warning(
                        f"   Parent may have been deleted or failed to create"
                    )
                    return None
            ...
            if "foreign key constraint" in str(e) and "parent_id" in str(e):
                logger.warning(
                    f"⚠️  [CHILD_PAGE_SKIP] Parent was deleted during processing"
                )
                logger.warning(f"   URL: {page_url}")
                logger.warning(f"   Parent ID: {parent_id}")
                logger.warning(
                    f"   This can happen if parent was deleted while children were being scraped"
                )
                return None
    ...
    async def update_website_with_page_data(
        ...
        logger.info(
            f"💾 [UPDATE_WEBSITE_PAGE] Updating website {website_id} with page data"
        )
        logger.info(f"   Title: {title}")
        ...
        try:
            ...
            async with get_db_session() as session:
                ...
                if result:
                    ...
                    logger.info(
                        f"   Status transition: {result.old_status} -> {result.new_status}"
                    )
                    await _invalidate_kb_file_list_cache()

                if mark_completed:
                    logger.info(
                        f"✅ [UPDATE_WEBSITE_PAGE_SUCCESS] Website record updated and marked as completed"
                    )
                else:
                    logger.info(
                        f"✅ [UPDATE_WEBSITE_PAGE_SUCCESS] Website record updated (status unchanged)"
                    )
                logger.log_db_query(query, params, "UPDATE succeeded")
                return True
        except Exception as e:
            logger.error(
                f"❌ [UPDATE_WEBSITE_PAGE_ERROR] Failed to update website: {e}"
            )
            logger.error(f"   Website ID: {website_id}")
            import traceback
    ...
    async def check_and_update_parent_completion(
        ...
        """
        logger.info(
            f"🔍 [PARENT_CHECK] Checking completion status for parent {parent_id}"
        )
        ...
        try:
            async with get_db_session() as session:
                ...
                if not update_result:
                    ...
                    logger.info(
                        f"ℹ️ [PARENT_CHECK] Parent {parent_id} not found or already in terminal state"
                    )
                    return False
                ...
                if total == 0:
                    ...
                    logger.info(
                        f"⚠️ [PARENT_PARTIAL] Parent {parent_id}: {completed} completed, {failed} failed out of {total}"
                    )
                    logger.info(
                        f"✅ [PARENT_UPDATE] Parent {parent_id} status: {update_result.old_status} -> {update_result.new_status} (with {failed} failures)"
                    )
            ...
            logger.error(
                f"❌ [PARENT_CHECK_ERROR] Failed to check parent completion: {e}"
            )
            import traceback
    ...
    async def get_websites_by_source_type(
