...
class AtomicDeletionService:
    ...
    async def delete_file_atomically(self, file_id: str) -> Dict[str, Any]:
        ...
        with self.tracer.start_as_current_span("atomic_file_deletion") as span:
            ...
            logger.info(
                f"🗑️ [ATOMIC_DELETE] Starting atomic file deletion for ID: {file_id}"
            )
            ...
            try:
                ...
                async with get_db_session() as session:
                    ...
                    if celery_task_id and file_record["processing_status"] in (
                        "pending",
                        "processing",
                    ):
                    ...
                    if file_record["processed_content_s3_key"]:
                        s3_processed_deleted = await self._delete_from_s3(
                            file_record["processed_content_s3_key"]
                        )
                        span.set_attribute(
                            "s3_processed_deleted", str(s3_processed_deleted)
                        )
                        logger.info(
                            f"🧹 [S3_CLEANUP] Deleted retained processed markdown: {file_record['processed_content_s3_key']}"
                        )
                    ...
                    try:
                        ...
                        logger.info(
                            f"🧹 [VECTOR_CLEANUP] Deleted {chunks_deleted} chunks from document_chunks for file {file_id}"
                        )
                    ...
                    logger.info(
                        f"✅ [ATOMIC_DELETE] File {file_id} deleted successfully"
                    )
    ...
    async def delete_website_atomically(self, website_id: str) -> Dict[str, Any]:
        ...
        with self.tracer.start_as_current_span("atomic_website_deletion") as span:
            ...
            logger.info(
                f"🗑️ [ATOMIC_DELETE] Starting atomic website deletion for ID: {website_id}"
            )
            ...
            try:
                async with get_db_connection() as conn:
                    ...
                    async with conn.transaction():
                        ...
                        logger.info(
                            f"🌐 [WEBSITE_DETAILS] Found: {website_record['original_url']}"
                        )
                        logger.info(f"   Status: {website_record['processing_status']}")
                        logger.info(
                            f"   Celery Task ID: {website_record['celery_task_id']}"
                        )
                        logger.info(
                            f"   Storage Document: {website_record['storage_document_name']}"
                        )
                        logger.info(
                            f"   S3 Processed: {website_record['processed_content_s3_key']}"
                        )

                        # Step 2: Cancel Celery task if processing
                        celery_task_id = website_record["celery_task_id"]
                        if celery_task_id and website_record["processing_status"] in (
                            "pending",
                            "processing",
                        ):
                        ...
                        if website_record["processed_content_s3_key"]:
                            s3_processed_deleted = await self._delete_from_s3(
                                website_record["processed_content_s3_key"]
                            )
                            span.set_attribute(
                                "s3_processed_deleted", str(s3_processed_deleted)
                            )
                            logger.info(
                                f"🧹 [S3_CLEANUP] Deleted retained processed markdown: {website_record['processed_content_s3_key']}"
                            )
                        ...
                        try:
                            ...
                            chunks_deleted = (
                                await vector_dao.delete_chunks_for_document(
                                    website_id, "website"
                                )
                            )
                            logger.info(
                                f"🧹 [VECTOR_CLEANUP] Deleted {chunks_deleted} chunks from document_chunks for website {website_id}"
                            )
                        except Exception as vec_err:
                            logger.warning(
                        ...
                        logger.info(
                            f"✅ [ATOMIC_DELETE] Website {website_id} deleted successfully"
                        )
                        logger.info(f"   S3 processed deleted: {s3_processed_deleted}")
                ...
                logger.error(
                    f"❌ [ATOMIC_DELETE] Error deleting website {website_id}: {e}"
                )
                span.set_attribute("error", str(e))
                return {
                    "success": False,
    ...
    async def _cancel_celery_task(self, celery_task_id: str, task_type: str) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"⚠️ [CELERY_CANCEL] Could not revoke task {celery_task_id}: {revoke_err}"
                )

            redis_conn.close()
            logger.info(
                f"✅ [CELERY_CANCEL] Set cancellation flag for task {celery_task_id}"
            )
            return True

        except Exception as e:
            logger.error(
                f"❌ [CELERY_CANCEL] Error cancelling task {celery_task_id}: {e}"
            )
            return False
