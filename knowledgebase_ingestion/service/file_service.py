    ...
    async def check_duplicate_file(self, original_filename: str, sha256_hash: Optional[str] = None) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            async with get_db_connection() as conn:
                # Check by filename first - only consider active files (exclude deleted, failed, cancelled)
                logger.info(f"🔍 [FILENAME_CHECK] Searching for filename match: {original_filename}")
                existing_by_name = await conn.fetchrow(
                    "SELECT id, original_filename, display_name, sha256_hash, file_size, gemini_file_name, version FROM file_uploads WHERE original_filename = $1 AND processing_status NOT IN ('deleted', 'failed', 'cancelled')",
                    original_filename
                )
                if existing_by_name:
                ...
                if sha256_hash:
                    ...
                    existing_by_hash = await conn.fetchrow(
                        "SELECT id, original_filename, display_name, sha256_hash, file_size, gemini_file_name, version FROM file_uploads WHERE sha256_hash = $1 AND processing_status NOT IN ('deleted', 'failed', 'cancelled')",
                        sha256_hash
                    )
                    if existing_by_hash:
