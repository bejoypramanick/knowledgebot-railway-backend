-- Complete Database Schema for Knowledgebot
-- This single file contains all CREATE and INSERT statements
-- All table modifications are integrated into CREATE statements
...
CREATE INDEX idx_user_role_mapping_is_active ON public.user_role_mapping USING btree (is_active);
...
-- Integrated with docling columns from add_docling_columns.sql
...
-- public.service_health_checks definition
-- Integrated from migrations/add_service_health_checks.sql
...
-- Create the admin user with both admin and human_agent roles
