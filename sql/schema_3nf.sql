...
CREATE TABLE token_usage_log (
    ...
    api_call_type VARCHAR(50), -- 'chat', 'rag', 'sentiment', 'summary'
    request_metadata JSONB, -- additional request details
    created_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP
);

CREATE INDEX idx_token_usage_log_session ON token_usage_log(session_id);
CREATE INDEX idx_token_usage_log_provider ON token_usage_log(provider);
CREATE INDEX idx_token_usage_log_created_at ON token_usage_log(created_at DESC);
