...
CREATE INDEX IF NOT EXISTS idx_file_uploads_docling ON public.file_uploads USING btree (id DESC) WHERE (processed_by_docling = true);
CREATE INDEX IF NOT EXISTS idx_file_uploads_docling_perf ON public.file_uploads USING btree (docling_processing_time_ms DESC) INCLUDE (docling_images_extracted, docling_images_with_ocr, processed_by_docling) WHERE ((processed_by_docling = true) AND (docling_processing_time_ms > 0));
