...
CREATE TABLE IF NOT EXISTS public.file_uploads ( id uuid DEFAULT uuidv7() NOT NULL, user_role_id uuid NULL, original_filename varchar(500) NOT NULL, display_name varchar(500) NULL, file_extension varchar(50) NULL, storage_document_name varchar(500) NULL, storage_document_uri text NULL, storage_backend_state varchar(50) DEFAULT 'pending'::character varying NULL, sha256_hash varchar(64) NULL, file_size int8 NULL, mime_type varchar(100) NULL, metadata jsonb DEFAULT '{}'::jsonb NULL, "version" int4 DEFAULT 1 NULL, processed_by_extractor bool DEFAULT false NULL, extractor_processing_time_ms int4 NULL, extractor_images_extracted int4 DEFAULT 0 NULL, extractor_images_with_ocr int4 DEFAULT 0 NULL, processing_status varchar(20) DEFAULT 'pending'::character varying NULL, error_message text NULL, s3_key text NULL, processed_content_s3_key text NULL, celery_task_id varchar(255) NULL, created_at timestamptz DEFAULT CURRENT_TIMESTAMP NULL, updated_at timestamptz DEFAULT CURRENT_TIMESTAMP NULL, char_count int4 DEFAULT 0 NULL, file_category varchar(50) GENERATED ALWAYS AS (
CASE
    ...
    ELSE 'other'::text
END) STORED NULL, is_successful bool GENERATED ALWAYS AS (processing_status::text = 'completed'::text) STORED NULL, embedding_character_count int4 DEFAULT 0 NULL, embedding_word_count int4 DEFAULT 0 NULL, embedding_token_count int4 DEFAULT 0 NULL, total_pages int4 DEFAULT 0 NULL, CONSTRAINT file_uploads_pkey PRIMARY KEY (id), CONSTRAINT valid_processing_status CHECK (((processing_status)::text = ANY (ARRAY['pending'::text, 'processing'::text, 'completed'::text, 'failed'::text, 'cancelled'::text, 'deleted'::text]))), CONSTRAINT file_uploads_user_role_id_fkey FOREIGN KEY (user_role_id) REFERENCES public.user_role_mapping(user_role_id) ON DELETE SET NULL);
CREATE INDEX IF NOT EXISTS idx_file_uploads_completed_storage_lookup ON public.file_uploads USING btree (storage_document_name) INCLUDE (display_name, processed_content_s3_key, file_size, file_category) WHERE ((processing_status)::text = 'completed'::text);
...
CREATE INDEX IF NOT EXISTS idx_file_uploads_storage_sync ON public.file_uploads USING btree (storage_backend_state, created_at DESC) INCLUDE (storage_document_uri, s3_key, storage_document_name) WHERE ((storage_backend_state)::text <> 'completed'::text);
