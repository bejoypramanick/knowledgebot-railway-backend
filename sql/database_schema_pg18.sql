-- ============================================================================
-- PG18 UUIDv7 DATABASE SCHEMA - Knowledgebot Platform
-- ============================================================================
-- PostgreSQL 18 schema using native uuidv7() for primary keys.
-- All tables use `id uuid DEFAULT uuidv7() PRIMARY KEY` EXCEPT
-- widget_configuration which keeps `id serial4` with CHECK (id = 1).
--
...
-- - 1 sequence (widget_configuration_id_seq only)
...
--
-- USAGE: psql -U postgres -d knowledgebot -f database_schema_pg18.sql
...
COMMENT ON SCHEMA public IS '3NF Normalized Schema (PG18 UUIDv7) - All tables follow Third Normal Form. Complete audit trail, role-based access control, and session management.';

-- ============================================================================
-- SEQUENCES (Only widget_configuration retains serial/sequence)
-- ============================================================================

-- Widget configuration (single-row table, keeps serial4)
...
CREATE TABLE IF NOT EXISTS public.chat_messages (
	...
	system_prompt_char_count int4 DEFAULT 0 NULL,
	system_prompt_word_count int4 DEFAULT 0 NULL,
	system_prompt_token_count int4 DEFAULT 0 NULL,
	history_char_count int4 DEFAULT 0 NULL,
	history_word_count int4 DEFAULT 0 NULL,
	history_token_count int4 DEFAULT 0 NULL,
	tool_def_char_count int4 DEFAULT 0 NULL,
	tool_def_word_count int4 DEFAULT 0 NULL,
	tool_def_token_count int4 DEFAULT 0 NULL,
	system_prompt_text text DEFAULT '' NULL,
	history_text text DEFAULT '' NULL,
	tool_def_text text DEFAULT '' NULL,
	user_msg_char_count int4 DEFAULT 0 NULL,
	user_msg_word_count int4 DEFAULT 0 NULL,
	user_msg_token_count int4 DEFAULT 0 NULL,
	bot_response_char_count int4 DEFAULT 0 NULL,
	bot_response_word_count int4 DEFAULT 0 NULL,
	bot_response_token_count int4 DEFAULT 0 NULL,
	created_at timestamptz DEFAULT CURRENT_TIMESTAMP NULL,
	updated_at timestamptz DEFAULT CURRENT_TIMESTAMP NULL,
...
-- Widget configuration (single row - id=1, keeps serial4)
...
-- Widget suggested messages (widget_config_id stays int4 since widget_configuration.id is serial4)
...
-- Notification settings (added proper PK constraint)
...
-- Insert default roles (no hardcoded IDs, use ON CONFLICT on role_name)
...
-- Insert default personas (no hardcoded IDs, use ON CONFLICT on persona_name)
...
-- Create admin user and assign roles (uses uuid lookups, no hardcoded integer IDs)
DO $$
DECLARE
    user_id_val UUID;
    admin_role_id UUID;
    human_agent_role_id UUID;
BEGIN
