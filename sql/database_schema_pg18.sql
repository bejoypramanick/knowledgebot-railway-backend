...
CASE
    ...
    ELSE 'other'::text
END) STORED NULL, is_successful bool GENERATED ALWAYS AS (processing_status::text = 'completed'::text) STORED NULL, filestore_character_count int4 DEFAULT 0 NULL, filestore_word_count int4 DEFAULT 0 NULL, filestore_token_count int4 DEFAULT 0 NULL, total_pages int4 DEFAULT 0 NULL, CONSTRAINT file_uploads_pkey PRIMARY KEY (id), CONSTRAINT valid_processing_status CHECK (((processing_status)::text = ANY (ARRAY['pending'::text, 'processing'::text, 'completed'::text, 'failed'::text, 'cancelled'::text, 'deleted'::text]))), CONSTRAINT file_uploads_user_role_id_fkey FOREIGN KEY (user_role_id) REFERENCES public.user_role_mapping(user_role_id) ON DELETE SET NULL);
